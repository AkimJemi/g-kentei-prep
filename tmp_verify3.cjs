...
const checks = {
    ...
    '08': ['ResNet', 'YOLO', 'CLIP', 'Pix2Pix', 'Cycle GAN', '基盤モデル', 'SHAP', 'LIME', '蒸留', '量子化', '宝くじ', 'VGGNet', 'Grad-CAM', 'プルーニング', 'NeRF', 'Zero Shot'],
...
Object.entries(filePatterns).forEach(([ch, pattern]) => {
    ...
    if (!fname) { lines.push(`Ch${ch}: NOT FOUND for "${pattern}"`); return; }

    let text = '';
    try { text = fs.readFileSync(path.join(dir, fname), 'utf8'); }
    catch (e) { lines.push(`Ch${ch}: READ ERROR ${e.message}`); return; }

    const kws = checks[ch] || [];
    const found = kws.filter(k => text.includes(k));
    const missing = kws.filter(k => !text.includes(k));
    lines.push(`\nCh${ch} [${fname}] ${text.length}chars`);
    lines.push(`  OK: ${found.join(', ')}`);
    if (missing.length) lines.push(`  MISS: ${missing.join(', ')}`);
});

fs.writeFileSync('./tmp_verify_result.txt', lines.join('\n'), 'utf8');
console.log('Written to tmp_verify_result.txt');
