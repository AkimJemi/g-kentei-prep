    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center justify-between border-b border-white/[0.04] pb-8">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-white italic uppercase tracking-tight flex items-center gap-4">
            <Bell className="w-8 h-8 text-accent shrink-0" />
            システム通知 (System Alerts)
          </h1>
          <p className="text-slate-500 font-medium tracking-tight">運営からの重要なお知らせおよびステータスアップデート</p>
        </div>
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((note) => (
              <motion.div 
                ...
                className={clsx(
                  "p-6 rounded-2xl border transition-all duration-300 flex gap-6 items-start group relative overflow-hidden",
                  note.isRead 
