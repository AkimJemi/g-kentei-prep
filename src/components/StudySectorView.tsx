...
export const StudySectorView: React.FC<StudyModeProps> = ({ onStartPractice, onShowQuestionList }) => {
        ...
        {categories.map((cat, idx) => {
          ...
          return (
            <motion.div
              ...
              onClick={() => isAuthenticated && onStartPractice(cat.id)}
              className={clsx(
                "group relative p-4 md:p-8 bg-secondary/10 border border-white/[0.04] rounded-2xl md:rounded-3xl text-left transition-all shadow-xl overflow-hidden flex flex-col h-full",
                isAuthenticated
                  ? "hover:bg-secondary/20 hover:border-accent/30 cursor-pointer"
            ...
            >
              ...
              <div className="flex gap-2 mt-3 relative z-10">
                {isAuthenticated ? (
                  ...
                  <div className="w-full flex items-center justify-center gap-2 py-2 bg-slate-800/60 border border-slate-700/30 rounded-xl text-slate-500 text-[10px] font-black uppercase tracking-widest">
                    <span>🔒</span>
                    <span>ログイン後使用可能</span>
                  </div>
                )}
