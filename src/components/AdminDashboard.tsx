...
export const AdminDashboard: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'dashboard' | 'users' | 'messages' | 'submissions' | 'questions' | 'notifications' | 'tasks' | 'categories' | 'errors'>('dashboard');
    ...
    useEffect(() => {
        const tabIds = ['dashboard', 'users', 'messages', 'submissions', 'questions', 'categories', 'notifications', 'tasks', 'errors'];
    ...
    return (
        <div className="flex h-[calc(100vh-80px)] bg-slate-950/20 overflow-hidden">
            {/* Left Sidebar */}
            <aside className="w-20 lg:w-72 border-r border-white/5 bg-slate-900/40 backdrop-blur-xl flex flex-col shrink-0">
                {/* Sidebar Header */}
                <div className="p-6 border-b border-white/5 hidden lg:block">
                    <h2 className="text-xl font-black text-white italic tracking-tighter uppercase">Admin <span className="text-accent underline decoration-accent/30">Protocol</span></h2>
                    <p className="text-[9px] font-mono text-slate-500 uppercase tracking-widest mt-1">G-KENTEI PREP CMS</p>
                </div>

                {/* Navigation Items */}
                <nav className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-1 custom-scrollbar">
                    {[
                        { id: 'dashboard', label: 'ダッシュボード', icon: TrendingUp, shortcut: '0' },
                        ...
                        <button
                            ...
                            className={clsx(
                                ...
                                activeTab === tab.id
                                    ? "bg-accent/10 border border-accent/20 text-accent shadow-[0_0_20px_rgba(56,189,248,0.1)]"
                                    : "text-slate-500 hover:text-slate-300 hover:bg-white/5"
                            )}
                        >
                            <tab.icon className={clsx("w-5 h-5", activeTab === tab.id ? "text-accent" : "text-slate-500")} />
                            <span className="hidden lg:block text-xs font-bold whitespace-nowrap">{tab.label}</span>

                            {/* Unread badges in sidebar */}
                            {tab.id === 'messages' && adminStats.unreadMessages > 0 && (
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 bg-blue-500 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-black animate-pulse">
                            ...
                            {/* Shortcut hint */}
                            <span className="hidden lg:block absolute right-4 text-[8px] font-mono text-slate-700 opacity-0 group-hover:opacity-100 transition-opacity">
                                [{tab.shortcut}]
                            </span>
                        </button>
                    ))}
            ...
            {/* Main Content Area */}
            <main className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 lg:p-12 space-y-8">
                {/* Page Title & Desc */}
                <div className="space-y-1 mb-8">
                    <h1 className="text-2xl md:text-4xl font-black text-white italic tracking-tight uppercase leading-none">
                        {activeTab === 'dashboard' ? 'SYSTEM OVERVIEW' : activeTab.toUpperCase()}
                    </h1>
                    <p className="text-[10px] md:text-xs text-slate-500 uppercase tracking-[0.3em] font-medium">管理者ダッシュボード • {activeTab}</p>
                </div>

                {/* DASHBOARD TAB (Global Summary & Visualizations) */}
                {activeTab === 'dashboard' && (
                ...
                {/* CONTENT AREA (Single Page Layout for Data Management) */}
                {activeTab !== 'dashboard' && (
                    <div className="bg-secondary/10 border border-white/[0.04] rounded-3xl overflow-hidden shadow-2xl min-h-[400px] animate-fade-in flex flex-col">
                                            ...
                                            {errorLogs.map(err => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-8 py-6">
                                                        ...
                                                        <pre className="text-[10px] text-slate-500 font-mono bg-slate-900/50 p-2 rounded max-w-[400px] overflow-x-auto max-h-[100px] custom-scrollbar">
                                                            {err.errorStack}
                                                        </pre>
                                                    </td>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-lg w-full space-y-6">
                            ...
                            <textarea
                                ...
                                onChange={(e) => setReplyModal(prev => prev ? { ...prev, replyText: e.target.value } : null)}
                        ...
                        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-2xl w-full overflow-y-auto max-h-[90vh]">
                            ...
                            <form onSubmit={handleSaveQuestion} className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <textarea rows={3} value={questionModal?.data.explanation || ''} onChange={(e) => setQuestionModal(prev => prev ? { ...prev, data: { ...prev.data, explanation: e.target.value } } : null)} className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white" />
                                </div>
