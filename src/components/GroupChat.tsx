...
export const GroupChat: React.FC = () => {
    ...
    const handleDeleteMessage = async (messageId: number) => {
        ...
        if (!confirm('【警告】このメッセージをニューラル・アーカイブから永久にパージしますか？この操作は取り消せません。')) return;
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert('【エラー】プロトコル失敗。メッセージのパージに失敗しました。');
            }
        } catch (error) {
                ...
                <AnimatePresence>
                    {(messages.length === 0 && !isFocused) && (
                        <motion.div 
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="absolute inset-0 z-0 p-8 flex flex-col justify-center pointer-events-none"
                        >
                            <div className="text-slate-400 text-sm space-y-8 opacity-40">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="bg-slate-800/20 p-5 rounded-2xl border border-slate-700/30">
                                        <h5 className="font-bold text-sky-400/50 mb-3 flex items-center gap-2 text-xs uppercase tracking-widest">
                                            <span className="text-lg">⚡</span> Quick Actions
                                        </h5>
                                        <ul className="space-y-3 text-xs">
                                            <li className="flex items-center justify-between">
                                                <span>演習を開始</span>
                                                <kbd className="bg-slate-900/50 border border-slate-700/30 px-2 py-0.5 rounded text-[10px] text-slate-500 font-mono">E / Enter</kbd>
                                            </li>
                                            <li className="flex items-center justify-between">
                                                <span>分析レポート</span>
                                                <kbd className="bg-slate-900/50 border border-slate-700/30 px-2 py-0.5 rounded text-[10px] text-slate-500 font-mono">Q</kbd>
                                            </li>
                                            <li className="flex items-center justify-between">
                                                <span>弱点克服モード</span>
                                                <kbd className="bg-slate-900/50 border border-slate-700/30 px-2 py-0.5 rounded text-[10px] text-slate-500 font-mono">W</kbd>
                                            </li>
                                        </ul>
                                    </div>
                                    ...
                                    <div className="bg-slate-800/20 p-5 rounded-2xl border border-slate-700/30">
                                        <h5 className="font-bold text-emerald-400/50 mb-3 flex items-center gap-2 text-xs uppercase tracking-widest">
                                            <span className="text-lg">🤝</span> Guidelines
                                        </h5>
                                        <ul className="space-y-3 text-xs text-slate-500">
                                            <li className="flex items-start gap-2">
                                                <span className="opacity-50 translate-y-0.5">📢</span>
                                                <span className="leading-snug">
                                                    <strong className="block mb-0.5">進捗をシェア</strong>
                                                    今日の学習成果を報告しましょう。
                                                </span>
                                            </li>
                                            <li className="flex items-start gap-2">
                                                <span className="opacity-50 translate-y-0.5">💡</span>
                                                <span className="leading-snug">
                                                    <strong className="block mb-0.5">質問・相談</strong>
                                                    分からない用語はここで解決。
                                                </span>
                                            </li>
                                        </ul>
                                ...
                                <div className="bg-gradient-to-r from-indigo-500/5 via-purple-500/5 to-pink-500/5 p-4 rounded-xl border border-indigo-500/10 text-center">
                                    <p className="text-[10px] text-indigo-300/40 font-mono mb-2 uppercase tracking-wider">System Ready</p>
                                        ...
                                        <div className="flex items-center gap-2 mb-1 px-1">
                                            ...
                                            {canDelete && (
                                                <button 
                                                    ...
                                                    className="opacity-0 group-hover/msg:opacity-100 p-1 hover:bg-red-500/20 rounded text-slate-700 hover:text-red-500 transition-all"
                                                >
