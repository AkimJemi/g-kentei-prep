...
const explanations = {
    ...
    41: [ // Classification
        "データがどのクラス（カテゴリ）に属するかを予測するタスクです。",
        "回帰は連続する数値を予測するタスクです。",
        "クラスタリングはデータ間の類似度に基づいてグループ分けする教師なし学習です。",
        "次元削減はデータの特徴量を減らす処理です。"
    ],
    42: [ // Regression
        "分類は連続しないカテゴリを予測するタスクです。",
        "明日の気温や売上など、連続する数値（実数値）を予測するタスクです。",
        "クラスタリングはデータの構造を捉えてグループ化する手法です。",
        "次元削減はデータの圧縮や可視化に使われます。"
    ],
    49: [ // SVM
        "データ点との距離（マージン）を最大化するように決定境界（ひきかた）を決める強力な手法です。",
        "PCA（主成分分析）は次元削減の手法です。",
        "LDA（線形判別分析）はクラス間の分散を最大化するように射影する手法です。",
        "決定木は条件分岐を組み合わせて予測する手法です。"
    ],
    50: [ // PCA
        "クラスタリングは共通点を持つデータを集めてグループを作る手法です。",
        "データの分散を最大化して情報を保持しつつ、多次元から低次元へと圧縮する手法です。",
        "回帰は連続した数値の変化を予測する手法です。",
        "ブースティングは弱い学習器を組み合わせて精度を高めるアンサンブル手法です。"
    ]
};
